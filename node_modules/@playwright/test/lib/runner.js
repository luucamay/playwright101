...
class Runner {
  ...
  async _runFiles(list, filesByProject, testFileReFilters) {
    ...
    if (!total) fatalErrors.push(createNoTestsError()); // 8. Fail when output fails.

    await Promise.all(Array.from(outputDirs).map(outputDir => removeFolderAsync(outputDir).catch(e => {
      fatalErrors.push((0, _util2.serializeError)(e));
    }))); // 9. Compute shards.
    ...
    config.__testGroupsCount = testGroups.length; // 10. Report begin

    (_this$_reporter$onBeg = (_this$_reporter3 = this._reporter).onBegin) === null || _this$_reporter$onBeg === void 0 ? void 0 : _this$_reporter$onBeg.call(_this$_reporter3, config, rootSuite); // 11. Bail out on errors prior to running global setup.
